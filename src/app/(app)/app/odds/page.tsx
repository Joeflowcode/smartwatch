...
export default async function OddsPage({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-[family-name:var(--font-brand)] text-3xl font-semibold">Odds comparison</h1>
        <p className="mt-1 text-sm text-[var(--muted-foreground)]">
          ...
          {meta.isMock ? (
            <Badge className="ml-2 border-amber-500/40 text-amber-700 dark:text-amber-300">
              Mock data · {new Date(meta.fetchedAt).toLocaleString()}
            </Badge>
          ) : null}
        </p>
      </div>
      ...
      <form className="flex flex-wrap gap-3 text-sm">
        ...
        <button
          type="submit"
          className="h-10 rounded-md bg-[var(--primary)] px-4 text-[var(--primary-foreground)]"
        >
          Filter
        </button>
      </form>
            ...
            <Card key={event.id}>
              ...
              <CardContent className="overflow-x-auto">
                <table className="w-full min-w-[640px] text-left text-sm">
                  ...
                  <tbody>
                    {[...bySelection.entries()].flatMap(([, quotes]) => {
